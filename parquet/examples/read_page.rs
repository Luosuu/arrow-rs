...
use parquet::file::direct_page::{
    generate_random_page_indices_file_level, get_file_page_locations, read_record_from_page,
};
...
fn main() -> Result<()> {
    ...
    let random_page_indices =
        generate_random_page_indices_file_level(page_location_matrix, column_idx).unwrap(); // (row_group_index, page_index)
    ...
    for (row_group_index, page_index) in random_page_indices {
        ...
        .unwrap()
        .unwrap();
        // input_ids are identical to labels for GPT-like tokenized dataset.
        ...
        //     .unwrap();
        //
        // if _array_labels != _array_input_ids {
        //     println!("labels are not same with input_ids!");
        // }
    }
